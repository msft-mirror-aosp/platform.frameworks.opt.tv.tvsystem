...
public class WatchdogStorage {
    ...
    /**
     * The database is clean when it is synchronized with the in-memory cache. Cannot start a write
     * while in this state.
     */
    private static final int DB_STATE_CLEAN = 1;

    /**
     * The database is dirty when it is not synchronized with the in-memory cache. When the database
     * is in this state, no write is in progress.
     */
    ...
    public boolean startWrite() {
        synchronized (mLock) {
            if (mCurrentDbState != DB_STATE_DIRTY) {
                Slogf.e(
                        TAG,
                        "Cannot start a new write while the DB state is %s",
    ...
    public void markWriteSuccessful() {
        synchronized (mLock) {
            if (mCurrentDbState != DB_STATE_WRITE_IN_PROGRESS) {
                Slogf.e(
                        TAG,
                        "Failed to mark write successful as the current db state is %s",
                        toDbStateString(mCurrentDbState));
                return;
            }
            ...
            try {
                ...
                for (int i = 0; i < entries.size(); ++i) {
                    ...
                    if (mUserPackagesByKey.get(UserPackage.getKey(entry.userId, entry.packageName))
                                    != null
                            && UserPackageSettingsTable.updateEntry(db, entry)) {
                        continue;
                    }
    ...
    public List<UserPackageSettingsEntry> getUserPackageSettings() {
        ...
        for (int i = 0; i < entriesById.size(); ++i) {
            ...
            UserPackage userPackage =
                    new UserPackage(userPackageId, entry.userId, entry.packageName);
     ...
     *
     * @return the number of saved entries, on success. Otherwise, returns {@code
     *     FAILED_TRANSACTION}
     */
    ...
    public List<IoUsageStatsEntry> getTodayIoUsageStats() {
        synchronized (mLock) {
            ...
            for (int i = 0; i < ioUsagesById.size(); ++i) {
                ...
                mTodayIoUsageStatsEntries.add(
                        new IoUsageStatsEntry(
                                userPackage.userId,
    ...
    public void deleteUserPackage(@UserIdInt int userId, String packageName) {
        ...
        UserPackageSettingsTable.deleteUserPackage(
                getDatabase(/* isWritable= */ true), userId, packageName);
    }

    /**
     * Returns the aggregated historical I/O overuse stats for the given user package or {@code
     * null} when stats are not available.
     */
    @Nullable
        ...
        for (int i = 0; i < dailyIoUsageSummaries.size(); i++) {
            ...
            systemTotalWrittenBytes +=
                    writtenBytes.getForegroundBytes()
                            ...
                            + writtenBytes.getGarageModeBytes();
        }
    ...
    public @Nullable List<UserPackageDailySummaries> getTopUsersDailyIoUsageSummaries(
        ...
        long systemTotalWrittenBytes =
                IoUsageStatsTable.querySystemTotalWrittenBytes(
                        db, includingStartEpochSeconds, excludingEndEpochSeconds);
        if (systemTotalWrittenBytes < minSystemTotalWrittenBytes) {
            return null;
        }
        summariesById =
                IoUsageStatsTable.queryTopUsersDailyIoUsageSummaries(
                        db, numTopUsers, includingStartEpochSeconds, excludingEndEpochSeconds);
        ...
        for (int i = 0; i < summariesById.size(); ++i) {
            ...
            userPackageDailySummaries.add(
                    new UserPackageDailySummaries(
                            userPackage.userId, userPackage.packageName, summariesById.valueAt(i)));
        }
        userPackageDailySummaries.sort(
                Comparator.comparingLong(UserPackageDailySummaries::getTotalWrittenBytes)
                        .reversed());
    ...
    public List<NotForgivenOverusesEntry> getNotForgivenHistoricalIoOveruses(int numDaysAgo) {
        ...
        for (int i = 0; i < notForgivenOverusesById.size(); i++) {
            ...
            notForgivenOverusesEntries.add(
                    new NotForgivenOverusesEntry(
                            userPackage.userId,
                            userPackage.packageName,
                            notForgivenOverusesById.valueAt(i)));
        }
        return notForgivenOverusesEntries;
    }

    /**
     * Forgives all historical overuses between yesterday and {@code numDaysAgo} for a list of
     * specific {@code userIds} and {@code packageNames}.
     */
    public void forgiveHistoricalOveruses(
            SparseArray<List<String>> packagesByUserId, int numDaysAgo) {
        ...
        IoUsageStatsTable.forgiveHistoricalOverusesForPackage(
                getDatabase(/* isWritable= */ true),
                userPackageIds,
                includingStartEpochSeconds,
                excludingEndEpochSeconds);
        Trace.endSection();
    ...
    public void syncUsers(int[] aliveUserIds) {
        ...
        UserPackageSettingsTable.syncUserPackagesWithAliveUsers(
                getDatabase(/* isWritable= */ true), aliveUsers);
        Trace.endSection();
    }
    ...
    int saveIoUsageStats(List<IoUsageStatsEntry> entries, boolean shouldCheckRetention) {
        ...
        try {
            ...
            for (int i = 0; i < entries.size(); ++i) {
                ...
                UserPackage userPackage =
                        mUserPackagesByKey.get(UserPackage.getKey(entry.userId, entry.packageName));
                if (userPackage == null) {
                    Slogf.e(
                            TAG,
                            "Failed to find user package id for user id '%d' and package '%s",
                ...
                ZonedDateTime statsDate =
                        Instant.ofEpochSecond(ioOveruseStats.startTime)
                                .atZone(ZONE_OFFSET)
                                .truncatedTo(STATS_TEMPORAL_UNIT);
                if (shouldCheckRetention
                        && STATS_TEMPORAL_UNIT.between(statsDate, currentDate)
                                >= RETENTION_PERIOD.get(STATS_TEMPORAL_UNIT)) {
                    continue;
                }
                long statsDateEpochSeconds = statsDate.toEpochSecond();
                rows.add(
                        IoUsageStatsTable.getContentValues(
                                userPackage.userPackageId, entry, statsDateEpochSeconds));
            }
            return atomicReplaceEntries(
                    getDatabase(/* isWritable= */ true), IoUsageStatsTable.TABLE_NAME, rows);
        } finally {
            Trace.endSection();
        }
    }
    ...
    private SQLiteDatabase getDatabase(boolean isWritable) {
        ...
        SQLiteDatabase db =
                isWritable ? mDbHelper.getWritableDatabase() : mDbHelper.getReadableDatabase();
     ...
     *
     * @return the number of replaced entries, on success. Otherwise, returns {@code
     *     FAILED_TRANSACTION}
     */
    ...
    static final class UserPackageSettingsEntry {
        ...
        public String toString() {
            return new StringBuilder()
                    .append("UserPackageSettingsEntry{userId: ")
                    .append(userId)
                    .append(", packageName: ")
                    .append(packageName)
                    .append(", killableState: ")
                    .append(killableState)
                    .append('}')
                    .toString();
        }
    ...
    static final class UserPackageDailySummaries {
        ...
        public String toString() {
            return new StringBuilder()
                    .append("UserPackageDailySummaries{userId: ")
                    .append(userId)
                    .append(", packageName: ")
                    .append(packageName)
    ...
    static final class UserPackageSettingsTable {
        ...
        public static void createTable(SQLiteDatabase db) {
            ...
            Slogf.i(
                    TAG,
                    "Successfully created the %s table in the %s database version %d",
                    TABLE_NAME,
                    WatchdogDbHelper.DATABASE_NAME,
                    WatchdogDbHelper.DATABASE_VERSION);
        }
        ...
        public static boolean updateEntry(SQLiteDatabase db, UserPackageSettingsEntry entry) {
            ...
            values.put(
                    COLUMN_KILLABLE_STATE_LAST_MODIFIED_EPOCH,
                    entry.killableStateLastModifiedEpochSeconds);
            StringBuilder whereClause =
                    new StringBuilder(COLUMN_PACKAGE_NAME)
                            .append(" = ? AND ")
                            .append(COLUMN_USER_ID)
                            .append(" = ?");
            ...
            if (db.update(TABLE_NAME, values, whereClause.toString(), whereArgs) < 1) {
                Slogf.e(
                        TAG,
                        "Failed to update %s entry with package name: %s and user id: %d",
                        TABLE_NAME,
                        entry.packageName,
                        entry.userId);
                return false;
        ...
        public static ArrayMap<String, UserPackageSettingsEntry> querySettings(SQLiteDatabase db) {
            ...
            try (Cursor cursor = db.rawQuery(queryBuilder.toString(), new String[] {})) {
                ArrayMap<String, UserPackageSettingsEntry> entriesById =
                        new ArrayMap<>(cursor.getCount());
                while (cursor.moveToNext()) {
                    entriesById.put(
                            cursor.getString(0),
                            new UserPackageSettingsEntry(
                                    cursor.getInt(1),
                                    cursor.getString(2),
                                    cursor.getInt(3),
                                    cursor.getInt(4)));
        ...
        public static void deleteUserPackage(
                SQLiteDatabase db, @UserIdInt int userId, String packageName) {
            Trace.beginSection(
                    "WdStorage.deleteUserPackage(package="
                            + packageName
                            + ", userId="
                            + userId
                            + ")");
            ...
            Slogf.i(
                    TAG,
                    "Deleted %d user package settings db rows for user %d and package %s",
                    deletedRows,
                    userId,
                    packageName);
            Trace.endSection();
        ...
        public static void syncUserPackagesWithAliveUsers(SQLiteDatabase db, IntArray aliveUsers) {
            ...
            Slogf.i(
                    TAG,
                    "Deleted %d user package settings db rows while syncing with alive users",
                    deletedRows);
        }
    }
    ...
    static final class NotForgivenOverusesEntry {
        ...
        NotForgivenOverusesEntry(
                @UserIdInt int userId, String packageName, int notForgivenOveruses) {
            this.userId = userId;
            this.packageName = packageName;
        ...
        @Override
        public String toString() {
            return "NotForgivenOverusesEntry {UserId: "
                    + userId
                    + ", Package name: "
                    + packageName
                    + ", Not forgiven overuses: "
                    + notForgivenOveruses
                    + "}";
        }
    }
    ...
    static final class IoUsageStatsTable {
        ...
        public static void createTable(SQLiteDatabase db) {
            ...
            createCommand
                    ...
                    .append(" INTEGER NOT NULL, ")
                    .append(COLUMN_NUM_OVERUSES)
                    .append(" INTEGER NOT NULL, ")
                    .append(COLUMN_NUM_FORGIVEN_OVERUSES)
                    .append(" INTEGER NOT NULL, ")
                    .append(COLUMN_NUM_TIMES_KILLED)
                    .append(" INTEGER NOT NULL, ")
                    .append(COLUMN_WRITTEN_FOREGROUND_BYTES)
                    .append(" INTEGER, ")
                    .append(COLUMN_WRITTEN_BACKGROUND_BYTES)
                    .append(" INTEGER, ")
                    .append(COLUMN_WRITTEN_GARAGE_MODE_BYTES)
                    .append(" INTEGER, ")
                    .append(COLUMN_REMAINING_FOREGROUND_WRITE_BYTES)
                    .append(" INTEGER, ")
                    .append(COLUMN_REMAINING_BACKGROUND_WRITE_BYTES)
                    .append(" INTEGER, ")
                    .append(COLUMN_REMAINING_GARAGE_MODE_WRITE_BYTES)
                    .append(" INTEGER, ")
                    .append(COLUMN_FORGIVEN_FOREGROUND_WRITE_BYTES)
                    .append(" INTEGER, ")
                    .append(COLUMN_FORGIVEN_BACKGROUND_WRITE_BYTES)
                    .append(" INTEGER, ")
                    .append(COLUMN_FORGIVEN_GARAGE_MODE_WRITE_BYTES)
                    .append(" INTEGER, ")
                    .append("PRIMARY KEY (")
                    .append(COLUMN_USER_PACKAGE_ID)
                    .append(", ")
        ...
        public static ArrayMap<String, PackageIoUsage> queryStats(
        ...
        public static @Nullable IoOveruseStats queryIoOveruseStatsForUserPackageId(
            ...
            IoOveruseStats.Builder statsBuilder =
                    new IoOveruseStats.Builder(earliestEpochSecond, durationInSeconds);
        ...
        public static void forgiveHistoricalOverusesForPackage(
                SQLiteDatabase db,
                List<String> userPackageIds,
                long includingStartEpochSeconds,
                long excludingEndEpochSeconds) {
            ...
            StringBuilder updateQueryBuilder =
                    new StringBuilder("UPDATE ")
                            ...
                            .append(COLUMN_DATE_EPOCH)
                            .append(">= ")
                            .append(includingStartEpochSeconds)
                            .append(" and ")
                            .append(COLUMN_DATE_EPOCH)
                            .append("< ")
                            .append(excludingEndEpochSeconds);
            ...
            Slogf.i(
                    TAG,
                    "Attempted to forgive overuses for I/O usage stats entries on pid %d",
                    Process.myPid());
        }
        ...
        public static @Nullable List<AtomsProto.CarWatchdogDailyIoUsageSummary>
            ...
            queryBuilder
                    ...
                    .append(COLUMN_DATE_EPOCH)
                    .append(", '")
                    .append(DATE_MODIFIER)
                    .append("', '")
                    .append(ZONE_MODIFIER)
                    .append("') as stats_date_epoch ")
                    .append("FROM ")
                    .append(TABLE_NAME)
                    .append(" WHERE ")
                    .append(COLUMN_DATE_EPOCH)
        ...
        public static long querySystemTotalWrittenBytes(
                SQLiteDatabase db, long includingStartEpochSeconds, long excludingEndEpochSeconds) {
            StringBuilder queryBuilder = new StringBuilder();
            queryBuilder
                    .append("SELECT SUM(")
        ...
        public static @Nullable ArrayMap<String, List<AtomsProto.CarWatchdogDailyIoUsageSummary>>
            ...
            innerQueryBuilder
                    ...
                    .append(" GROUP BY ")
                    .append(COLUMN_USER_PACKAGE_ID)
                    .append(" ORDER BY total_written_bytes DESC LIMIT ")
                    .append(numTopUsers)
                    .append(')');
            StringBuilder queryBuilder = new StringBuilder();
            queryBuilder
    ...
    static final class WatchdogDbHelper extends SQLiteOpenHelper {
        ...
        WatchdogDbHelper(Context context, boolean useDataSystemCarDir, TimeSource timeSource) {
            ...
            super(
                    ...
                    useDataSystemCarDir
                            ? new File(TvWatchdogService.getWatchdogDirFile(), DATABASE_NAME)
                                    .getAbsolutePath()
                            : DATABASE_NAME,
                    /* name= */ null,
                    DATABASE_VERSION);
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
            ...
            try {
                while (upgradeVersion < currentVersion) {
                    switch (upgradeVersion) {
                        ...
                        default:
                            String errorMsg =
                                    "Tried upgrading to an invalid database version: "
                                            + upgradeVersion
                                            + " (current version: "
                                            + currentVersion
                                            + ")";
                ...
                Slogf.i(
                        TAG,
                        "Successfully upgraded database from version %d to %d",
                        oldVersion,
                        upgradeVersion);
            } finally {
                db.endTransaction();
            ...
            if (upgradeVersion != currentVersion) {
                Slogf.i(
                        TAG,
                        "Failed to upgrade database from version %d to %d. "
        ...
        private void upgradeToVersion3(SQLiteDatabase db) {
            ...
            execSql =
                    new StringBuilder("INSERT INTO ")
                            ...
                            .append(UserPackageSettingsTable.COLUMN_KILLABLE_STATE)
                            .append(", ")
                            .append(mTimeSource.getCurrentDate().toEpochSecond())
                            .append(" FROM ")
                            .append(oldUserPackageSettingsTable);
            db.execSQL(execSql.toString());
            execSql =
                    new StringBuilder("DROP TABLE IF EXISTS ").append(oldUserPackageSettingsTable);
            db.execSQL(execSql.toString());
            execSql =
                    new StringBuilder("INSERT INTO ")
                            .append(IoUsageStatsTable.TABLE_NAME)
                            ...
                            .append(oldIoUsageStatsTable);
            db.execSQL(execSql.toString());
            execSql = new StringBuilder("DROP TABLE IF EXISTS ").append(oldIoUsageStatsTable);
            db.execSQL(execSql.toString());
         ...
         * an auto-incrementing integer ID and uses the ID (instead of its rowid) as one of the
         * primary keys in {@link IoUsageStatsTable} along with a foreign key dependency.
         *
        ...
        private void upgradeToVersion2(SQLiteDatabase db) {
            ...
            db.execSQL(execSql.toString());
            execSql =
                    new StringBuilder("DROP TABLE IF EXISTS ").append(IoUsageStatsTable.TABLE_NAME);
            db.execSQL(execSql.toString());
        ...
        public static void createUserPackageSettingsTableV2(SQLiteDatabase db) {
            ...
            Slogf.i(
                    TAG,
                    "Successfully created the %s table in the %s database version %d",
                    UserPackageSettingsTable.TABLE_NAME,
                    WatchdogDbHelper.DATABASE_NAME,
                    2);
        }
    ...
    private static final class UserPackage {
        ...
        @Override
        public String toString() {
            return new StringBuilder("UserPackage{userPackageId: ")
                    .append(userPackageId)
                    .append(", userId: ")
                    .append(userId)
                    .append(", packageName: ")
                    .append(packageName)
